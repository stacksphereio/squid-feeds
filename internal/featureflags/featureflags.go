...
type Flags struct {
	...
	// Feed feature flags
	FeedsEnabled        server.RoxFlag // feeds.enabled
	FeedsWeatherEnabled server.RoxFlag // feeds.weather.enabled
	FeedsNewsEnabled    server.RoxFlag // feeds.news.enabled

	// Regional flags
	FeedsRegionEUEnabled   server.RoxFlag // feeds.region.eu.enabled
	FeedsRegionNAEnabled   server.RoxFlag // feeds.region.na.enabled
	FeedsRegionSAEnabled   server.RoxFlag // feeds.region.sa.enabled
	FeedsRegionAsiaEnabled server.RoxFlag // feeds.region.asia.enabled
}
...
var (
	flags = &Flags{
		...
		FeedsEnabled:           server.NewRoxFlag(false),
		FeedsWeatherEnabled:    server.NewRoxFlag(false),
		FeedsNewsEnabled:       server.NewRoxFlag(false),
		FeedsRegionEUEnabled:   server.NewRoxFlag(false),
		FeedsRegionNAEnabled:   server.NewRoxFlag(false),
		FeedsRegionSAEnabled:   server.NewRoxFlag(false),
		FeedsRegionAsiaEnabled: server.NewRoxFlag(false),
	}
